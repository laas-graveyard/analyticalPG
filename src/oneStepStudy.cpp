...
void ConeStepStudy::buildNecessaryInternalStructures()
{
  ...
  mp_aHRCD->LoadStructureForTheHumanoid(); //warning hands are not
...
void ConeStepStudy::initAndGenOpenHRPFiles()
{
  ...
  if(mp_leftOrRightFirstStable == 'R') {
   ...
   if(mp_generate) {    
     ...
     replace_value_vectorN(mp_sseq.size(), mp_pythonScript.c_str(), search2.c_str(), mp_sseq, "");
...
void ConeStepStudy::runPGAndEvalTrajectory() {
  ...
  double minJointsLimits[30] = { 
    -45.0, -35.0, -125.0, -2.0, -75.0, -20.0, // right leg
    -30.0, -20.0, -125.0, -2.0, -75.0, -35.0,  // left leg
    
    -45.0, -5.0, // chest 
    -45.0 -30.0, // head
    
    -180.0, -95.0, -92.0, -137.0, -92.0, -92.0, -60.0, // right arm
    -180.0, -10.0, -92.0, -137.0, -92.0, -92.0, -60.0 // left arm 
  };
      
  double maxJointsLimits[30] = { 
    30.0, 20.0, 42.0, 150.0, 42.0, 35.0, // right leg
    45.0, 35.0, 42.0, 150.0, 42.0, 20.0, // left leg
    
    45.0, 60.0,  // chest 
    45.0, 45.0,  // head
    
    60.0, 10.0, 92.0, 2.0, 92.0, 92.0, 16.0, // right arm
    60.0, 95.0, 92.0, 2.0, 92.0, 92.0, 16.0 // left arm 
  };
  ...
  while(
	...
	if(support_type != support_type_past) { //either one foot just
						//rose, or one foot
						//just landed
	  trigger = 0;
	}
